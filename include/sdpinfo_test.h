...
var Capabilities = map[string]*Capability{
	"audio": &Capability{
		Codecs: vector<string>{"opus"},
	},
	"video": &Capability{
		Codecs: vector<string>{"vp8"},
		Rtx:    true,
		Rtcpfbs: []*RtcpFeedback{
			...
			&RtcpFeedback{
				ID:     "ccm",
				Params: vector<string>{"fir"},
			},
			&RtcpFeedback{
				ID:     "nack",
				Params: vector<string>{"pli"},
			},
		},
...
func Test_SDPAnswer(t *testing.T) {
	...
	if answer.GetMedia("audio") == nullptr {
		t.Error("answer audio error")
	}

	if answer.GetMedia("video") == nullptr {
